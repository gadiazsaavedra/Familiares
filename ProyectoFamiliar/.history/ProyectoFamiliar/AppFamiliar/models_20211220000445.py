...
class Padre(models.Model):
    ...
    def __str__(self):
        return f"PADRE: {self.nombre} APELLIDO: {self.apellido} TELEFONO: {self.telefono} DIRECCION: {self.direccion} CORREO: {self.correo} FECHA_NACIMIENTO: {self.fecha_nacimiento} PARENTESCO: {self.parentesco}"
    
class Hijo(models.Model):
    apellido = models.CharField(max_length=50)
    numero = models.IntegerField()
    esBueno = models.BooleanField(default=True)
