...
from AppFamiliar.forms import AbuelosFormulario, PrimoFormulario, HijoFormulario, UserRegister
...
def hijoFormulario(request):
    if request.method == 'POST':
        ...
        if miFormulario.is_valid():
                informacion = miFormulario.cleaned_data
                hij = Hijo(apellido=informacion['apellido'], numero=informacion['numero'], esBueno=informacion['esBueno'])               
                hij.save()
                return render(request, 'AppFamiliar/inicio.html')
    else:
